...
---
```{r include = FALSE}
knitr::opts_chunk$set(fig.height = 6, fig.width = 8, dpi = 90,fig.pos = "H")
counter <- 1
```
...
Generalized Lambdas Distribution().  
...
data.  
GLD method is based in the GLDEX package [@SteveSU].
...
Looking at the distribution plots above we can see that in some variables like
...
provided by *survival* package [@survival-package]. Cancer data set consists of 167 samples and
...
In the upcoming plot, the surv_fit() curves from the *survival* package for both
original and simulate data are depicted.  
...
plot(fit,
     col=1:2)
legend(700, 1,
       c("Original data set", "Simulate data set"),
       lty=c(1,1),
       col=c(1,2),
       bty='n',
       lwd=2)
